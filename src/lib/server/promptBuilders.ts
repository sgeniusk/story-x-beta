...
import type { EpisodeLengthContract } from '../storyEngine';
...
// PLAY 실시간 대화를 한 회차 원고로 장면화하는 응결 프롬프트.
// 로컬 CLI가 실제 생성 경로이므로 tools/storyx.mjs 의 buildDiveCondensePrompt와 byte-identical하게 유지한다.
export interface DiveCondensePromptInput {
  character: string;
  scene: string;
  context: string;
  transcript: string;
...
// PLAY 응결 프롬프트 — storyx.mjs 의 buildDiveCondensePrompt와 전체 결과 byte-identical 유지.
export function buildDiveCondensePrompt(input: DiveCondensePromptInput): string {
  const { character, scene, context, transcript, arc, episodeLength } = input;
