...
const DIVE_CONDENSE_SOURCE_SAFETY_RULES = [
...
describe('buildDiveCondensePrompt — PLAY 응결 장면화·보이스 품질 계약', () => {
  ...
  it.each(Object.values(EPISODE_LENGTH_CONTRACTS))(
    '$preset 프리셋의 목표·허용 범위·생성 안전 범위·장면 수를 동적으로 지시한다',
    (episodeLength) => {
      const prompt = buildDiveCondensePrompt({ ...input, episodeLength });
      const n = (value: number) => value.toLocaleString('ko-KR');
      expect(prompt).toContain(`- 본문은 한국어 기준 목표 ${n(episodeLength.targetChars)}자, 허용 범위 ${n(episodeLength.minChars)}~${n(episodeLength.maxChars)}자로 씁니다.`);
      expect(prompt).toContain(`- 원문 근거가 목표 분량을 뒷받침할 때의 최종 허용 범위는 ${n(episodeLength.minChars)}~${n(episodeLength.maxChars)}자이며, 모델의 글자 수 계산 편차를 고려한 생성 목표는 ${n(episodeLength.generationMinChars)}~${n(episodeLength.generationMaxChars)}자입니다.`);
      expect(prompt).toContain(`- 원문 근거에 각 장면을 뒷받침하는 사건·행동이 있을 때만 서로 다른 압력의 현재 장면 ${episodeLength.minScenes}~${episodeLength.maxScenes}개까지 재구성합니다. 근거가 부족하면 장면 수를 줄이고, 사건 보고나 줄거리 요약으로 대신하지 않습니다.`);
      expect(prompt).toContain(`prose 글자 수를 공백 문자를 제외하고 직접 확인합니다. ${n(episodeLength.minChars)}자 미만이면`);
      expect(prompt).toContain(`${n(episodeLength.maxChars)}자를 넘어도 자동으로 자르지 않습니다.`);
    }
  );

  it('희소 원문의 8천 자 목표도 장면 수·갈등 연쇄를 조건부로 두고 새 사건 발명 대신 짧게 반환한다', () => {
    const prompt = buildDiveCondensePrompt({
      character: '한서윤 — 선배.',
      scene: '비 오는 회사 옥상 문 앞',
      context: '',
      transcript: '나: 문을 바라봤다.',
      arc: '',
      episodeLength: EPISODE_LENGTH_CONTRACTS.extended
    });

    expect(prompt).toContain('- 원문 근거가 시도·반격·관계 또는 지위 비용·주요 폭로(reveal)·강제 선택을 각각 뒷받침할 때만 하나의 중앙 갈등 안에서 그 연쇄를 깊게 합니다. 일부만 뒷받침되면 확인된 단계까지만 쓰고 나머지는 만들지 않습니다.');
    for (const rule of DIVE_CONDENSE_SOURCE_SAFETY_RULES) expect(prompt).toContain(rule);
    expect(prompt).not.toContain('- 서로 다른 압력을 가진 현재 장면 4~6개로 재구성합니다.');
    expect(prompt).not.toContain('- 하나의 중앙 갈등 안에서 시도 → 반격 → 관계·지위 비용 → 주 reveal 하나 → 강제 선택을 깊게 합니다.');
    expect(prompt).not.toContain('- 최종 허용 범위는 7,200~8,800자로 유지하되');
    expect(prompt).not.toContain('긴장·다음 전개를 반영해 페이오프 있는 회차로');
  });
  ...
  it('비어 있는 장면·아크·캐릭터·기존 기억에는 명시적 fallback을 쓴다', () => {
    const prompt = buildDiveCondensePrompt({ character: '', scene: '', context: '', transcript: '', arc: '', episodeLength: EPISODE_LENGTH_CONTRACTS.standard });
