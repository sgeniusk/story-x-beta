...
describe('Story X version log', () => {
  it('publishes the current alpha version as a reusable product constant', () => {
    expect(STORYX_VERSION.label).toBe('Alpha v0.7.0');
    expect(STORYX_VERSION.version).toBe('0.7.0');
    expect(STORYX_VERSION.channel).toBe('alpha');
    expect(STORYX_VERSION.latestCommit).toBe('9691421');
    expect(STORYX_VERSION.testProof).toContain('106 tests');
  });
  ...
  it('keeps a compact milestone log for every major alpha improvement', () => {
    ...
    expect(storyxVersionLog[0].title).toContain('명령 팔레트');
    expect(storyxVersionLog[0].changes.join(' ')).toContain('⌘K');
  });
  ...
  it('documents the current roadmap baseline for future releases', () => {
    ...
    expect(docs).toContain('현재 기준 버전: `Alpha v0.7.0`');
    expect(docs).toContain('v0.8.0');
    expect(docs).toContain('v1.0.0-alpha');
    expect(docs).toContain('커밋: `9691421`');
  });
});
