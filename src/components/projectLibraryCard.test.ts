...
function entry(lifecycle: ProjectLibraryEntry['lifecycle']): ProjectLibraryEntry {
  const project = { ...createEmptyProject({ title: '젖은 어깨' }), id: 'work-1', logline: '막차가 끊긴 역에서 시작되는 이야기' };
  return {
...
describe('ProjectLibraryCard', () => {
  ...
  it('renders a confirmed work without a promotion action', () => {
    const html = renderToStaticMarkup(createElement(ProjectLibraryCard, {
      entry: entry('confirmed'), active: false, onOpen: () => {}, onConfirm: () => {}
    }));
