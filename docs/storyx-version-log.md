...
### Alpha v0.7.1 · 에디터 회차 탭 레이아웃 핫픽스

- 날짜: 2026-05-15
- 커밋: `43b60ac`
- 검증: `npm test` 24 files / 111 tests, `npm run build` pass

변경:

- 원고 화면의 작업대 행 구조를 원고 모드 전용 4행 레이아웃으로 분리했다.
- `1화` 회차 탭이 620px 원고 행을 차지하며 세로 막대처럼 늘어나는 문제를 막았다.
- 작품 바이블과 출간 준비 화면은 불필요한 빈 grid row를 만들지 않도록 별도 모드 클래스를 사용한다.

다음:

- 에디터 상단 액션 밀도와 원고 본문 폭을 계속 다듬는다.
