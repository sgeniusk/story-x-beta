...
## 2026-06-08 (이어서 2) — #2 9·10·11화 테스트 + 각 5인 검토 / 다종 실증

> 사용자 "테스트 계속" 선택(세션 방향 질문에 응답) → 9·10화 후 "커밋 + 11화 계속" 선택. #2 9·10·11화 라이브 생성 + 각 5인 검토 풀라이브. 방침대로 발견은 기록만. **코드 변경 0(라이브 테스트만).** 로그 = `docs/reviews/2026-06-07-persona-live-test/02-romancefantasy-regression.md` 의 `## 9·10·11화` 절.
...
- **11화 "지워진 오른편"(2070자, 45초)** — 연속성 ★★★★★. **캐논 고정 이름을 추리 제약으로 사용**(압흔 끝 "B(벨로트)" → 레나=W·루시안=L+날짜로 소거). L.B.=계승되는 자리로 심화. canonFacts 40→44. 5인검토 차단0·[수정]. **품질 게이트 6/8(이번 세션 최고).**
- **★ 실증** — (1) **캐논화 후 고정 5연속**(루시안 ch7~11 드리프트0) (2) **검토 일관성**(세계 키퍼=권한/접근비용 **ch9·10·11 3연속**·연속성=확정강도/공개범위 일관 = 랜덤 아닌 캐논 근거 반복검토) (3) **캐논 고정 이름이 미스터리 논리 규정**(ch11) (4) **품질게이트 본문반응**(5/8→5/8→6/8, hook gate 회차별 변동) (5) **미스터리 자가교정**(ch9 우려→ch10 해소) (6) **신규 수렴**(ch11 캐릭터·연속성이 리아나 미래지식 공개범위 독립 포착).
- P1(쇼러너 빈응답) 이번 0/3(3회 정상). 생성시간 단조증가 아님(ch10 39s·ch11 45s — codex 지연 변동).
...
- **테스트 계속** — #2 12화~ 완권(~20~25화) + ~10회 수정 사이클 → 6축 정식 점수 → 종합 리포트 → 새 제작 계획. (#2 진입 = `?stage=editor`, localStorage 보존됨. 다음 화 = 11화 잠금→produceEpisode.)
- **일괄 수정 묶음(테스트 후)** — P1 빈응답 가드(간헐 잔존, 이번 0/3) · 관계(relations) 추출 · P5 가족 드리프트(백필 + extractEntityName 서술부 추출 + 검토 characters 대조). **주의 — 세계/연속성이 반복 지적한 "접근 권한 미설명"은 본문 약점이지 코드 결함 아님(검토가 잘 잡는 중).**

### 손대지 말 것
- #2 작품 localStorage(이제 **11화까지·1~10화 locked**·characters [레나,리아나]·canonFacts 44). 발견 재현·연속성 보존 위해 9~11화 본문/드리프트 수정 안 함. P3·P2·P4 수정·테스트.

### 커밋
- 9·10화 docs 커밋됨 `aaf6d41`. 11화 테스트 = 코드 변경 0. 로그·handoff·progress(docs)만 추가 커밋 예정.
