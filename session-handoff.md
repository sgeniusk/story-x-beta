...
## 2026-06-08 (이어서 2) — #2 9·10화 테스트 + 각 5인 검토 / 4종 실증

> 사용자 "테스트 계속" 선택(세션 방향 질문에 응답). #2 9·10화 라이브 생성 + 각 5인 검토 풀라이브. 방침대로 발견은 기록만. **코드 변경 0(라이브 테스트만).** 로그 = `docs/reviews/2026-06-07-persona-live-test/02-romancefantasy-regression.md` 의 `## 9·10화` 절.

### 한 일
- **9화 "오른편으로 돌아가는 종이"(2418자)** — 8화 잠금(출간 경유, P2 수정으로 새로고침 없이 produceEpisode 전환) → 의도 메모 빈 값 → 캐논 digest만으로 생성. 연속성 ★★★★★. **L.B. 단서 대도약** — L.B.를 인명 아닌 "벨로트 백작의 오른편" 권한자 약호로 재해석. canonFacts 32→37. 5인검토 차단0·[수정]. **3명(세계·연속성·캐릭터)이 "루시안 기록실 접근 권한 미설명"을 독립 포착(차별점 실작동).**
- **10화 "오른편의 첫 날짜"(2110자, 39초)** — 연속성 ★★★★★. **ch9 검토 우려를 캐논으로 자연 해소**(첫 날짜엔 루시안이 오른편 전 → 배제). 최초 오른편 이름 칼훼손·첫 획 "레/르"만 남김 = 레나 회귀 떡밥. canonFacts 37→40. 5인검토 차단0·[수정].
- **★ 4종 실증** — (1) **캐논화 후 고정 4연속**(루시안 ch7~10 드리프트0) (2) **검토 일관성**(세계=기록실 접근권한·연속성=확정강도를 ch9→ch10 일관 포착 = 랜덤 아닌 캐논 근거 반복검토) (3) **품질게이트 본문반응**(hook_last_200 ch9 FAIL→ch10 PASS, hook_first_300 반대로 뒤집힘 = rank3 게이트가 본문 읽음) (4) **미스터리 자가교정**(캐논 digest가 ch9 검토우려를 ch10에서 해소).
- P1(쇼러너 빈응답) 이번 0/2(2회 정상). 생성시간 단조증가 아님(ch10 39초 — codex 지연 변동으로 정정).

### 검증
- `bash init.sh` 세션 시작·종료 2회 녹색(tsc·vitest·build). **코드 변경 0.** 라이브 콘솔 0. 캡처 `02/storyx-ch9-00·01*.png`(검토 화면 스크린샷은 MCP 간헐 타임아웃으로 evaluate 데이터로 대체 — 직전 세션과 동일).

### 다음 — 사용자 방침: 테스트 완료 후 일괄 수정
- **테스트 계속** — #2 11화~ 완권(~20~25화) + ~10회 수정 사이클 → 6축 정식 점수 → 종합 리포트 → 새 제작 계획. (#2 진입 = `?stage=editor`, localStorage 보존됨.)
- **일괄 수정 묶음(테스트 후)** — P1 빈응답 가드(간헐 잔존) · 관계(relations) 추출 · P5 가족 드리프트(백필 + extractEntityName 서술부 추출 + 검토 characters 대조). **주의 — 세계/연속성이 반복 지적한 "기록실 접근 권한 미설명"은 본문 약점이지 코드 결함 아님(검토가 잘 잡는 중).**

### 손대지 말 것
- #2 작품 localStorage(이제 **10화까지·1~9화 locked**·characters [레나,리아나]·canonFacts 40). 발견 재현·연속성 보존 위해 9·10화 본문/드리프트 수정 안 함. P3·P2·P4 수정·테스트.

### 커밋
- 9·10화 테스트 = 코드 변경 0. 로그·handoff·progress(docs)만 커밋 예정.
