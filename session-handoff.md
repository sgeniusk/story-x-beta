...
## 2026-06-08 (이어서 2) — #2 9~12화 테스트 + 각 5인 검토 / 다종 실증 + P6 신규

> 사용자 "테스트 계속" → "커밋+11화 계속" → "계속"(×2). #2 9~12화 라이브 생성 + 각 5인 검토 풀라이브. 방침대로 발견은 기록만. **코드 변경 0(라이브 테스트만).** 로그 = `docs/reviews/2026-06-07-persona-live-test/02-romancefantasy-regression.md` 의 `## 9~12화` 절. (세션 중 dev 서버 1회 사망→`npm run dev` 재시작, #2 localStorage 무손실.)
...
- **12화 "첫 오른편의 약속"(3000자=상한, 54초)** — 연속성 ★★★★★. **미스터리 페이오프** — 첫 배신자=레오르 벨로트(장례 명부상 죽은 벨로트), 첫 회송 날짜가 사망일 이후 = "죽은 자의 약속을 산 자가 지킴". 리아나 가짜 봉투 덫. canonFacts 44→48. 5인검토 **통과 우세(관찰3·수정1·결정1)** 이나 품질 게이트 4/8(hook 둘 다 FAIL) = **게이트↔페르소나 분기.** **★ P6 신규 — 인물 승격 시 "레오르 벨로트라"(조사 "라" 오염).**
- **★ 실증** — (1) **캐논화 후 고정 6연속**(루시안 ch7~12 드리프트0) (2) **검토 일관성**(세계 키퍼=권한/접근비용 **ch9~12 4연속** = 랜덤 아닌 캐논 근거 반복검토) (3) **캐논 고정 이름이 미스터리 논리 규정** → ch12 페이오프 (4) **게이트 본문반응** 5/8→5/8→6/8→4/8 + ch12 게이트↔페르소나 분기(상보적 두 렌즈) (5) **미스터리 자가교정**(ch9 우려→ch10·ch11 우려→ch12).
- P1(쇼러너 빈응답) 이번 0/4(4회 정상). 생성시간 단조증가 아님(ch10 39·ch11 45·ch12 54s — codex 지연 변동).
...
- **테스트 계속** — #2 13화~ 완권(~20~25화) + ~10회 수정 사이클 → 6축 정식 점수 → 종합 리포트 → 새 제작 계획. (#2 진입 = `?stage=editor`, localStorage 보존됨. 다음 화 = 12화 잠금→produceEpisode. dev 서버는 `npm run dev`로 살아 있어야 함 — 죽으면 about:blank.)
...
### 손대지 말 것
- #2 작품 localStorage(이제 **12화까지·1~11화 locked**·characters [레나,리아나,레오르 벨로트라]·canonFacts 48). 발견 재현·연속성 보존 위해 9~12화 본문/드리프트 수정 안 함(P6 "레오르 벨로트라" 오염도 그대로 둠 — 재현 보존). P3·P2·P4 수정·테스트.

### 커밋
- 9·10화 docs `aaf6d41`·11화 docs `8ff27bf` 커밋됨. 12화 테스트 = 코드 변경 0. 로그·handoff·progress(docs)만 추가 커밋 예정.
