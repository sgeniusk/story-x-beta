...
> Last Updated: 2026-06-08 · Branch: `main` (**#2 9~12화 테스트 + 5인검토 — 캐논화후 고정 6연속·검토 일관성 4연속·게이트↔페르소나 분기·미스터리 페이오프 · P6 신규(extractEntityName 조사) · 9·10화 `aaf6d41`·11화 `8ff27bf`** · #2 12화까지)
...
- **#2 9~12화 테스트 + 각 5인 검토 (2026-06-08 이어서, 코드변경 0)** — 9 "오른편으로 돌아가는 종이"·10 "오른편의 첫 날짜"·11 "지워진 오른편"·12 "첫 오른편의 약속" 생성+검토 풀라이브. 연속성 ★★★★★·오염 0·canonFacts 27→48·온톨로지 46→64·characters 2→3. **실증 — (1) 캐논화 후 고정 6연속(루시안 ch7~12) (2) 검토 일관성(세계 키퍼=권한/접근비용 ch9~12 4연속, 랜덤 아님) (3) 캐논 고정 이름이 미스터리 논리 규정 → ch12 페이오프(첫 배신자=장례명부의 죽은 벨로트 레오르) (4) 게이트 본문반응 5/8→5/8→6/8→4/8 + ch12 게이트(4/8)↔페르소나(통과우세) 분기 = 두 평가 렌즈 상보성 (5) 미스터리 자가교정(ch9 우려→ch10·ch11 우려→ch12 해소).** **P6 신규 — extractEntityName 조사 버그**(ch12 "레오르 벨로트라" 조사 "라" 오염). P1 이번 0/4. 9·10 `aaf6d41`·11 `8ff27bf` 커밋. 로그 `## 9~12화` 절.
- **다음** — (사용자 방침) P1·관계·P5·P6·floating 2c·2d 는 **실증 테스트 완료 후 일괄 수정** → #2 13화+ 완권·수정사이클·6축 → 종합 리포트 → 새 제작 계획.
...
## 최근 검증 (2026-06-08 · #2 9~12화 테스트 — 코드변경 0)

```
init.sh            → tsc · vitest(43 files) · build 전체 통과 (세션 중 3회. 9~12화는 코드 변경 없음)
#2 9~12화 라이브   → 연속성 ★★★★★·오염 0·canonFacts 32→48·온톨로지 46→64·characters 2→3. 캐논화후 고정 6연속(루시안 ch7~12). 5인검토 각 차단0. 생성 ch10 39·ch11 45·ch12 54초(단조증가 아님). 콘솔 0
실증                → 캐논화후고정6연속 · 검토일관성(세계 키퍼 ch9~12 4연속) · 캐논이름이 미스터리논리 규정→ch12 페이오프 · 게이트본문반응 5/8→5/8→6/8→4/8 + 게이트↔페르소나 분기 · 미스터리 자가교정 · P1 0/4
